...
func normalizeMachineNameCandidate(name string) string {
...
func shortenMachineNameLabel(name string) string {
	if len(name) <= labelHostnameLength {
		return name
	}
	hash := sha1.Sum([]byte(name))
	hashSuffix := hex.EncodeToString(hash[:])[:MachineGivenNameHashLength]
	maxPrefixLen := labelHostnameLength - MachineGivenNameTrimSize - MachineGivenNameHashLength
	if maxPrefixLen < 1 {
		maxPrefixLen = labelHostnameLength
	}
	prefix := strings.TrimRight(name[:maxPrefixLen], "-")
	if prefix == "" {
		prefix = "machine"
	}
...
func normalizeAutoGeneratedMachineName(name string) string {
	name = normalizeMachineNameCandidate(name)
	if name == "" {
		name = "machine"
	}
	return shortenMachineNameLabel(name)
}

func normalizeManualMachineName(name string) string {
	name = normalizeMachineNameCandidate(name)
	if name == "" {
		return ""
	}
...
func machineNameWithSuffix(base string, suffix int) string {
	suffixText := strconv.Itoa(suffix)
	if base == "" {
		base = "machine"
	}
	maxPrefixLen := labelHostnameLength - len(suffixText) - 1
	if maxPrefixLen < 1 {
		maxPrefixLen = 1
	}
	if len(base) > maxPrefixLen {
		base = strings.TrimRight(base[:maxPrefixLen], "-")
	}
	if base == "" {
		base = "machine"
		if len(base) > maxPrefixLen {
...
func (h *Mirage) GenMachineName(
	...
	baseName := normalizeAutoGeneratedMachineName(referName)
	giveName := baseName
	suffix := 1
	m, err := h.GetOrgMachineByGivenName(giveName, orgId)
	if err == nil && (m.MachineKey != machineKey || m.UserID != userId) {
		giveName = machineNameWithSuffix(baseName, suffix)
		m, err = h.GetOrgMachineByGivenName(giveName, orgId)
	}
...
func (h *Mirage) setAutoGenName(machine *Machine, newName string) (string, error) {
	...
	if newName == "" {
		...
		newName = normalizeManualMachineName(newName)
		if newName == "" {
...
func (h *Mirage) toNode(
	...
	if baseDomain == "" && machine.User.Organization.EnableMagic { //[cgao6 removed] dnsConfig != nil && dnsConfig.Proxied { // MagicDNS
