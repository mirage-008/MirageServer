...
<template>
  ...
  >
    ...
    >
      ...
      <p class="flex justify-start text-stone-500 mt-2 mb-8 text-xs text-left">
        *测试阶段仅支持手机号注册
      </p>
      <form @submit.prevent="$emit('confirm')">
        ...
        >
          ...
          <svg
            v-if="notifyName"
            t="1674704280252"
            class="icon h-9 -mr-16 animation-here"
            viewBox="0 0 1024 1024"
            version="1.1"
            xmlns="http://www.w3.org/2000/svg"
            p-id="3838"
        ...
        <div
          class="flex flex-row items-center justify-self-end mb-5 border rounded-md max-w-xs"
          :class="{
            'border-stone-300 hover:border-stone-400':
              sendCodeBtnDis && codeBtnText == '获取验证码',
            'border-green-400': !sendCodeBtnDis || codeBtnText != '获取验证码',
          }"
        >
          <input
            v-model="captchaInput"
            :disabled="!sendCodeBtnDis || (!regBtnDis && captchaInput == idCode)"
            @input="captchaCheck"
            type="text"
            name="captcha"
          ...
          />
          <svg
            v-if="!sendCodeBtnDis || codeBtnText != '获取验证码'"
            t="1674701202328"
            class="icon h-9 mr-20"
            viewBox="0 0 1024 1024"
            version="1.1"
            xmlns="http://www.w3.org/2000/svg"
            p-id="7795"
        ...
        <div class="flex flex-row justify-self-end mb-5 rounded-md max-w-xs">
          ...
          <VerifyCode
            :switchoff="regBtnDis"
            :setFocus="focusOnCode"
            @focusSet="focusOnCode = false"
            @complete="getNewVerifyCode"
          >
          </VerifyCode>
          <button
            ref="sendCodeBtn"
            :disabled="sendCodeBtnDis"
            @click="sendCode"
        ...
        <footer class="flex mt-10 justify-end space-x-4">
          ...
          <button
            ref="regBtn"
            :disabled="regBtnDis || codeInput.length < 6"
            @click="doRegister"
  ...
  <div
    v-if="preventAction"
    class="flex fixed overflow-y-auto inset-0 py-8 z-100 bg-gray-900 bg-opacity-[0.07]"
    style="pointer-events: auto"
  >
    <svg
      t="1674733336391"
      class="icon w-full animate-slowspin self-center"
      viewBox="0 0 1024 1024"
      version="1.1"
      xmlns="http://www.w3.org/2000/svg"
      p-id="9256"
    ...
    >
      <path
        d="M448 224a96 64 90 1 0 128 0 96 64 90 1 0-128 0Z"
        fill="#57534e"
        opacity=".9"
        p-id="9257"
      ></path>
      <path
        d="M448 800a96 64 90 1 0 128 0 96 64 90 1 0-128 0Z"
        fill="#57534e"
        opacity=".5"
        p-id="9258"
      ></path>
      <path
        d="M704 512a96 64 0 1 0 192 0 96 64 0 1 0-192 0Z"
        fill="#57534e"
        opacity=".7"
        p-id="9259"
      ></path>
      <path
        d="M128 512a96 64 0 1 0 192 0 96 64 0 1 0-192 0Z"
        fill="#57534e"
        opacity=".3"
        p-id="9260"
      ></path>
      ...
      ></path>
      <path
        d="M672.02313 760.903595a96 64 44.999 1 0 90.508088-90.511247 96 64 44.999 1 0-90.508088 90.511247Z"
        fill="#57534e"
        opacity=".6"
        p-id="9263"
      ></path>
      <path
        d="M261.468782 353.607652a96 64 44.999 1 0 90.508088-90.511247 96 64 44.999 1 0-90.508088 90.511247Z"
        fill="#57534e"
        opacity=".2"
        p-id="9264"
      ></path>
